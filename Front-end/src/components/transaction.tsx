...
import { ActivityIcon, ArrowDownCircle, ArrowUpCircle, HandCoins, PlusCircle, Trash, TrendingDown, TrendingUp, Wallet } from 'lucide-react';
type Transaction = {
  id: string;
  text: string;
  amount: number;
...
function Transaction() {
  ...
  const addTransaction = async () => {
    ...
    setLoading(true);
    try {
      await api.post<Transaction[]>('transactions/', { text, amount: Number(amount) });
  ...
  const dime = balance > 0 ? balance * 0.1 : 0;
